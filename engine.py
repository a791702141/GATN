...
class Engine(object):
    def __init__(self, state={}):
        ...
        self.state['images_num'] = 82081
        self.state['max_iterations'] = self.state['max_epochs'] * int((self.state['images_num'] / self.state['batch_size']) + 1)
    ...
    def train(self, data_loader, model, criterion, optimizer, epoch):
        ...
        for i, (input, target) in enumerate(data_loader):
            ...
            self.state['iteration'] = i + epoch * int((self.state['images_num'] / self.state['batch_size']) + 1)
...
class MultiLabelMAPEngine(Engine):
    ...
    def on_end_epoch(self, training, model, criterion, data_loader, optimizer=None, display=True):
        ...
        map_all = 100 * self.state['ap_meter'].value()
        loss = self.state['meter_loss'].value()[0]
...
class GCNMultiLabelMAPEngine(MultiLabelMAPEngine):
    def on_forward(self, training, model, criterion, data_loader, optimizer=None, display=True):
        if training:
            ...
            # compute output
            self.state['output'] = model(feature_var, inp_var, target_var, self.state['iteration'], self.state['max_iterations'])
            self.state['loss'] = criterion(self.state['output'], target_var)
