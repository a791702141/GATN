...
def gen_M(mods, dims=2048, t=0.999):
    n = mods.shape[0]
    m = int(max(mods)) - int(min(mods)) + 1
    if m <= 1:
        return np.ones((n, dims))
    x = []
    for i in range(n):
        for j in range(m):
            x.append(np.repeat(t if (mods[i] - int(min(mods))) == j else ((1 - t) / (m - 1)), dims))
    x = np.concatenate(x)
    return x.reshape((n, dims * m))
